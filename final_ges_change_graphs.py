...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 11 14:28:01 2020
...
def optimisation_gardanne(prix_charbon,prix_carbone):
    ...
    'charbon'         : [pcic/3.6,    1,    prix_charbon,  43.2,GRB.INFINITY,GRB.INFINITY,      3,      0,    0],
    ...
    cout_incorp_biomasse=model.addVar(lb=215,ub=430,vtype=GRB.CONTINUOUS)
    ...
    for i in range(durée):
        model.addConstr(0.5*sum(C[c,i] for c in combustible)>=sum(C[c,i] for c in combustible)-C['charbon',i]-300000000*X1[i])
        COU[i] = ((430*9.4) *(X1[i]) + (215*9.4)*(1-X1[i]))
    ...
    benef4 = sum(prix_carbone*Quota[i] for i in range(durée))
    ...
    for i in Quota:
        emission=emission+i
    emission_ges=int(LinExpr.getValue(emission))    
    ...
    for i in range(20):
        if i <5:
            quota.append(80000-ges_route_total_rv[i]-quicksum(ges[c]*C[c,i].x for c in combustible)-quicksum(route[c]*0.00016*C[c,i] for c in boisfrais)-quicksum(route[c]*0.00009*C[c,i].x for c in boistorrefie)-quicksum(mer[c]*0.000015*C[c,i].x for c in combustible))
        elif i >=5 and i <10:
            quota.append(60000-ges_route_total_rv[i]-quicksum(ges[c]*C[c,i].x for c in combustible)-quicksum(route[c]*0.00016*C[c,i] for c in boisfrais)-quicksum(route[c]*0.00009*C[c,i].x for c in boistorrefie)-quicksum(mer[c]*0.000015*C[c,i].x for c in combustible))
        elif i >=10 and i <15:
            quota.append(40000-ges_route_total_rv[i]-quicksum(ges[c]*C[c,i].x for c in combustible)-quicksum(route[c]*0.00016*C[c,i] for c in boisfrais)-quicksum(route[c]*0.00009*C[c,i].x for c in boistorrefie)-quicksum(mer[c]*0.000015*C[c,i].x for c in combustible))                                                             
        elif i >=15 :
            quota.append(20000-ges_route_total_rv[i]-quicksum(ges[c]*C[c,i].x for c in combustible)-quicksum(route[c]*0.00016*C[c,i] for c in boisfrais)-quicksum(route[c]*0.00009*C[c,i].x for c in boistorrefie)-quicksum(mer[c]*0.000015*C[c,i].x for c in combustible))
            #rajouter variabile binaire pour si pas de benef on le fait pas 
    s=0
    for i in quota:
        s=s+i
    ...
    stockage=LinExpr.getValue(sum(s50[i]*50000+s150[i]*150000 for i in range(20)))/1000
    ...
    return([Opti,emission_ges,stockage])
...
for i in range(150):
    for c,m in [('r','.'),('b','x')]:
        a=randrange(35,75)
        b=randrange(5,100)
        stockage[i]=optimisation_gardanne(a, b)[2]
        ax.scatter(a,b,benef[i],zdir='z',c=c,marker=m)

ax.set_xlabel('Prix du Charbon')
ax.set_ylabel('Prix du Carbone')
...
for i in range(150):
    b=randrange(5,100)
    benef[i]=optimisation_gardanne(45,b)[0]
    plt.scatter(b,benef[i])
...
for i in range(150):
    b=randrange(5,100)
    benef2[i]=optimisation_gardanne(40,b)[0]
    plt.scatter(b,benef2[i],'ro',marker='x')

benef=50*[0]
for i in range(50):
    a=randrange(77,125)
    benef[i]=optimisation_gardanne(60,20,a)
    plt.scatter(a,benef[i])


g.set_xlabel('Prix d electricité')  
g.set_ylabel('Bénéfice réalisé par lentreprise')  
plt.show()
